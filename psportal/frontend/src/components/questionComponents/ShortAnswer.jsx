...
export default function ShortAnswer({ config = {}, value = {}, onChange }) {
  ...
  const [correctAnswer, setCorrectAnswer] = useState(strOrEmpty(value?.correctAnswer));
  const [wordLimit, setWordLimit] = useState(strOrEmpty(value?.wordLimit));

  useEffect(() => {
    setQuestion(strOrEmpty(value?.question));
    setCorrectAnswer(strOrEmpty(value?.correctAnswer));
    setWordLimit(strOrEmpty(value?.wordLimit));
